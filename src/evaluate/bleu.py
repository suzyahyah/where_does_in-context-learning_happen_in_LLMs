...
def evaluate_sacrebleu(hyp_list, ref_list, bleu_c, bleu_s):
    # make tokenization function here.

    rref = [ref_list]
    hyp_list_ = []
    for h in hyp_list:
        if type(h) == str:
            # take the first sentence if doing MT.
            h = sent_tokenize(h)[0]
            hyp_list_.append(h)
        else:
    ...
    hyp_list = [h.replace("<|endoftext|>", "") for h in hyp_list]
...
def guess_tokenizer(hyp_fn, ref_df):

    if ("-ja." in hyp_fn) or ("-jpn." in hyp_fn):
        tokenizer = "ja-mecab"
    if "-zh" in hyp_fn:
        tokenizer = "zh"
    #elif len(ref_df['target'].iloc[0].split(' ')) < 10:
    #    tokenizer = "char"
    else:
        tokenizer = None
    print("tokenizer:", tokenizer)
        # none defaults to 13a
...
def main(hyp_fn, ref_fn):
    ...
    tokenizer = guess_tokenizer(hyp_fn, ref_df)
    ...
    if "doc_id" in merge_df.columns:
        ...
        for k, grp in merge_df.groupby('doc_id'):
            ...
            #hyp_list = postprocess_repetitions(hyp_list, ngrams=10)
            per_doc_scores, per_doc_dp_df = evaluate_sacrebleu(hyp_list, ref_list,
                                                               bleu_c, bleu_s)
...
if __name__ == "__main__":
    ...
    main(args.hyp_fn, args.ref_fn)# , tokenizer=args.tokenizer)
